return {
    ...
    config = function(_, opts)
        ...
        end, { desc = 'Telescope -> Find project commits [`<C-r>` checkout | `<C-r>m` mixed | `<C-r>s` soft | `<C-r>h` hard]' })
        vim.keymap.set('n', '<leader>gbc', function()
            local b = is_git_dir(cwd)
            if b then
                builtin.git_bcommits()
            end
        end, { desc = 'Telescope -> Find buffer commits [`<C-r>` checkout | <C-v>` vsplit diff | <C-x>` hsplit diff | <C-t>` tab diff]' })
