return {
    ...
    config = function()
        ...
        vim.keymap.set(
            ...
            "<leader>gs",
            vim.cmd.Git,
            { desc = "Fugitive -> Opens Fugitive buf, similar to git status" }
        )
        ...
        vim.api.nvim_create_autocmd("FileType", {
            ...
            callback = function()
                ...
                vim.keymap.set(
                    ...
                    "<leader>F",
                    "<cmd>diffget //2<CR>",
                    { buffer = bufnr, remap = false, desc = "Fugitive -> Select HEAD version on merge conflict" }
                )
                vim.keymap.set(
                    "n",
                    "<leader>J",
                    "<cmd>diffget //3<CR>",
                    { buffer = bufnr, remap = false, desc = "Fugitive -> Select BRANCH version on merge conflict" }
                )
            end,
        })
    end,
